void main(String[] args) throws IOException {
    ...
    for (List<String> order: orders) {
        ...
        // Everything is NOT ok, try to change the ordering so that it becomes ok
        List<String> correctedOrder = new ArrayList<>();
        for (String el: order) {
            for (int i = 0; i < correctedOrder.size(); i ++) {
                String existingEl = correctedOrder.get(i);
                boolean isLimited = IntStream
                        .range(0, first.size())
                        .anyMatch(index -> existingEl.equals(last.get(index)) && el.equals(first.get(index)));
                if (isLimited) {
                    correctedOrder.add(i, el);
                    break;
                }
            }
            if (!correctedOrder.contains(el)) {
                correctedOrder.add(el);
            }
        }
        result += Integer.parseInt(correctedOrder.get((correctedOrder.size() - 1) / 2));
    }
