...
class AdvancedPlayer {
    ...
    static int maxDepth;
    static Map<Long, Integer> cachedStates = new HashMap<>();
    ...
    static int compute(int[] curState, int depth) {
        long key = encodeState(curState, depth);

        // Check if we already have a result cached for the current state
        if (cachedStates.containsKey(key)) {
        ...
        for (int position = 0; position < 9; position ++) {
            ...
            if (curState[position] != 0) continue;
    ...
    static int encodeState(int[] curState, int depth) {
        int encodedState = 0;
        for (int digit : curState) {
            encodedState = encodedState * 7 + digit;
        }
        // Multiply by the maximal depth + 1 so that it can never overlap with other depths
        // For example, maxDepth = 10 and we would multiply by just 10:
        //      - state 123 at depth 10 => 123 * 10 + 10 = 1240
        //      - state 124 at depth 0  => 123 * 10 + 0 = 1240
        return encodedState * (maxDepth + 1) + depth;
    }
